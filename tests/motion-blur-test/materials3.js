...
(function() {
  ...
  core.inherit( tess.Material, tess.SoccerMaterial, {
    ...
    getProcessStatements: function( objects ) {
      ...
      return '' +
             // '      float motionBlurAngle = 0.15 * -0.0 / TWO_PI;\n' +
             '      float motionBlurAngle = ( times.x + ( times.y - times.x ) * ( pseudorandom(seed*14.53+1.6) ) ) / 11.0;\n' +
             '      mat2 motionBlurMatrix = mat2( cos(motionBlurAngle), -sin(motionBlurAngle), sin(motionBlurAngle), cos(motionBlurAngle) );\n' +
             '      mat2 motionBlurMatrixInv = mat2( cos(motionBlurAngle), sin(motionBlurAngle), -sin(motionBlurAngle), cos(motionBlurAngle) );\n' +
             '      vec3 rotNormal = normal;\n' +
             '      rotNormal.xy = motionBlurMatrix * rotNormal.xy;\n' +
             '      vec3 ico = closestIcosahedronPoint( rotNormal );\n' +
             '      vec3 dodeca = closestDodecahedronPoint( rotNormal );\n' +
             '      ico.xy = motionBlurMatrixInv * ico.xy;\n' +
             '      dodeca.xy = motionBlurMatrixInv * dodeca.xy;\n' +
